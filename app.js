...
function dashboard(){let b='<div class="dashboard-hero"><h2>VERTEX <span>Kontrol Merkezi</span></h2><p>Emlak, müşteri, teklif ve operasyon akışını tek merkezden yönetin. Aktif portföyleri, yaklaşan işleri ve sistem durumunu hızlıca görün.</p></div><div class="grid stats"><div class="card stat"><span>Aktif müşteri</span><strong>'+V.data.clients.length+'</strong></div><div class="card stat"><span>Devam eden iş</span><strong>'+V.data.jobs.length+'</strong></div><div class="card stat"><span>Aktif mülk</span><strong>'+V.data.properties.length+'</strong></div><div class="card stat"><span>Sistem sağlığı</span><strong class="ok">İyi</strong></div></div>'+sectorCards()+'<div class="grid cols" style="margin-top:20px"><div class="card"><div class="section-title"><h2>Yaklaşan işler</h2></div><div class="list">'+(V.data.tasks.length?V.data.tasks.map(x=>'<div class="row"><b>'+x.title+'</b><span>'+x.date+'</span><span class="pill">'+x.kind+'</span><span>›</span></div>').join(''):'<div class="empty-state">Henüz yaklaşan iş yok.<br><small>Teklif kabul edildiğinde işler burada görünür.</small></div>')+'</div></div><div class="card"><div class="section-title"><h2>Kontrol Merkezi</h2></div><p class="muted">Üç sektör birbirinden ayrı veri ve özellik kümeleriyle yönetilir.</p><div class="notice">Merkezi bulut veritabanı ve gerçek API bağlantıları sonraki altyapı aşamasında etkinleştirilecek. Bu dal şu an güvenli yönetim arayüzü prototipidir.</div></div></div>';return layout(b,'Kontrol Merkezi')}
...
async function forgotPassword(){const email='muratkullak@gmail.com';const redirectTo=location.origin+location.pathname;const {error}=await CLOUD.auth.resetPasswordForEmail(email,{redirectTo});if(error){alert('Şifre sıfırlama e-postası gönderilemedi: '+error.message);return}alert('Şifre sıfırlama bağlantısı e-posta adresine gönderildi. Gelen kutunu ve spam klasörünü kontrol et.');}
