...
module.exports = async function handler(req, res) {
  ...
  // The browser already has the VERTEX Supabase session. We never expose the
  // refresh token to JavaScript: it is sent once to a short-lived, HttpOnly
  // handoff cookie and immediately consumed by /api/google/finalize.
  res.setHeader('Set-Cookie',[
    'vertex_google_state=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0',
    `vertex_google_handoff=${encodeURIComponent(token.refresh_token)}; Path=/api/google/finalize; HttpOnly; Secure; SameSite=Strict; Max-Age=120`
  ]);
