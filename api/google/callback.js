...
module.exports = async function handler(req, res) {
  ...
  res.setHeader('Set-Cookie', 'vertex_google_state=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0');
  if (!token.refresh_token) return res.status(400).send('Bağlantı kuruldu ancak refresh token dönmedi. Google erişimini kaldırıp yeniden bağlayın.');
  // Never expose tokens to the browser. Temporary integration milestone: confirm OAuth;
  // persistent encrypted token storage is added in the next backend step.
